...
class Messaround extends React.Component {
    constructor(props) {
        super(props);
    ...
    save() {
        socket.emit("saved", document.getElementById("canvas").toDataURL());
    }
    reset() {
        socket.emit("reset");
    }
    ...
    render() {
        const snowman = (
            <div className="content-container">
                ...
                <button onClick={this.save}>Save it to your profile</button>
            </div>
        );
        ...
        if (this.cvs) {
            ...
            ctx2.beginPath();
            ctx2.moveTo(
                this.updateStartingPoint[0].x ||
                    this.props.startingCoordinate[0].x,
                this.updateStartingPoint[0].y ||
                    this.props.startingCoordinate[0].y
            );
            ctx2.lineTo(
...
export default connect(mapStateToProps)(Messaround);
